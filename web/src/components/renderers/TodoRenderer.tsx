import { Circle, CheckCircle2, AlertCircle, Clock, ArrowRight } from 'lucide-react';
...
const statusIcon: Record<string, { icon: typeof Circle; className: string }> = {
  pending: { icon: Circle, className: 'text-neutral-400 dark:text-neutral-500' },
  in_progress: { icon: Clock, className: 'text-blue-500 dark:text-blue-400' },
  completed: { icon: CheckCircle2, className: 'text-green-500 dark:text-green-400' },
  cancelled: { icon: Circle, className: 'text-neutral-300 dark:text-neutral-600 line-through' },
};
        ...
        return (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <span className={isDone ? 'text-neutral-500 dark:text-neutral-400' : 'text-neutral-800 dark:text-neutral-200'}>
                {todo.content}
              </span>
            </div>
            <span className={`flex-shrink-0 text-xs font-medium ${priorityClass}`}>
