...
export const CodeMirrorEditor = forwardRef<CodeMirrorEditorHandle, CodeMirrorEditorProps>(
  function CodeMirrorEditor(
          ...
          EditorView.domEventHandlers({
            keydown: (event) => {
              ...
              if (isMod && event.key === "g") {
                event.preventDefault();
                containerRef.current?.dispatchEvent(new CustomEvent("editor-gotoline"));
                return true;
              }
              return false;
            },
    ...
    useEffect(() => {
      if (viewRef.current) {
        viewRef.current.dispatch({
          effects: themeCompartment.current.reconfigure(createCodeMirrorTheme(dark)),
        });
      }
    }, [dark]);
    ...
    useEffect(() => {
      if (viewRef.current) {
        viewRef.current.dispatch({
          effects: externalExtensionsCompartment.current.reconfigure(externalExtensions),
        });
      }
    }, [externalExtensions]);
    ...
    return (
      <div ref={containerRef} className={`codemirror-editor h-full overflow-auto ${className}`} />
    );
  },
);
