//! Agent factory functions — create configured agent definitions.
//!
//! Provides helpers for constructing [`AgentDefinition`] instances with
//! optional overrides from configuration.
...
mod tests {
    ...
    fn test_create_agent_with_override() {
        let model = ActiveModel {
            ...
            api_type: tidev_config::ApiType::OpenAiChatCompletions,
    ...
    #[test]
    fn test_create_all_agents_count() {
        let all = create_all_agents();
        assert_eq!(all.len(), 6);
    }

    #[test]
    fn test_create_sub_agents_excludes_general() {
        let subs = create_sub_agents();
        assert_eq!(subs.len(), 5);
        assert!(subs.iter().all(|a| a.agent_type != AgentType::General));
    }
}
