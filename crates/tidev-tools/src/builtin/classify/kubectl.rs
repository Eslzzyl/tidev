...
pub(super) fn classify_kubectl(args: &[&str]) -> Safety {
    ...
    match sub {
        ...
        | "version" | "cluster-info" | "diff" | "options" | "help" | "completion" | "plugin" => {
            Safety::ReadOnly
        }
        ...
        "config" => {
            ...
            match action {
                "view" | "current-context" | "get-clusters" | "get-contexts" | "get-users"
                | "get-credentials" | "help" => Safety::ReadOnly,
        ...
        "apply" | "create" | "delete" | "edit" | "patch" | "replace" | "rollout" | "scale"
        | "autoscale" | "cordon" | "uncordon" | "drain" | "taint" | "label" | "annotate"
        | "port-forward" | "proxy" | "cp" | "auth" | "set" | "expose" | "run" | "attach"
        | "debug" => Safety::WriteOperation,
...
mod tests {
    ...
    fn kubectl_read_commands() {
        ...
        assert_eq!(
            classify_kubectl(&["get", "pods", "-o", "yaml"]),
            Safety::ReadOnly
        );
        assert_eq!(
            classify_kubectl(&["describe", "pod", "nginx"]),
            Safety::ReadOnly
        );
        ...
        assert_eq!(
            classify_kubectl(&["diff", "-f", "file.yaml"]),
            Safety::ReadOnly
        );
        assert_eq!(classify_kubectl(&["config", "view"]), Safety::ReadOnly);
        assert_eq!(
            classify_kubectl(&["config", "current-context"]),
            Safety::ReadOnly
        );
    }
    ...
    fn kubectl_write_commands() {
        assert_eq!(
            classify_kubectl(&["apply", "-f", "deploy.yaml"]),
            Safety::WriteOperation
        );
        assert_eq!(
            classify_kubectl(&["create", "deployment", "nginx"]),
            Safety::WriteOperation
        );
        assert_eq!(
            classify_kubectl(&["delete", "pod", "nginx"]),
            Safety::WriteOperation
        );
        assert_eq!(
            classify_kubectl(&["edit", "deployment/nginx"]),
            Safety::WriteOperation
        );
        assert_eq!(
            classify_kubectl(&["patch", "pod/nginx", "-p", "{}"]),
            Safety::WriteOperation
        );
        assert_eq!(
            classify_kubectl(&["rollout", "restart", "deploy/nginx"]),
            Safety::WriteOperation
        );
        assert_eq!(
            classify_kubectl(&["scale", "--replicas=3", "deploy/nginx"]),
            Safety::WriteOperation
        );
        assert_eq!(
            classify_kubectl(&["exec", "-it", "pod", "--", "bash"]),
            Safety::Unknown
        );
        assert_eq!(
            classify_kubectl(&["port-forward", "pod", "8080:80"]),
            Safety::WriteOperation
        );
        assert_eq!(
            classify_kubectl(&["label", "pod/nginx", "env=prod"]),
            Safety::WriteOperation
        );
        assert_eq!(
            classify_kubectl(&["annotate", "pod/nginx", "key=val"]),
            Safety::WriteOperation
        );
        assert_eq!(
            classify_kubectl(&["config", "set-context", "prod"]),
            Safety::WriteOperation
        );
        assert_eq!(
            classify_kubectl(&["config", "use-context", "prod"]),
            Safety::WriteOperation
        );
        assert_eq!(
            classify_kubectl(&["cordon", "node1"]),
            Safety::WriteOperation
        );
        assert_eq!(
            classify_kubectl(&["drain", "node1"]),
            Safety::WriteOperation
        );
    }
}
