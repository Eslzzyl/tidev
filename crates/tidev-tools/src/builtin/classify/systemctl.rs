...
pub(super) fn classify_systemctl(args: &[&str]) -> Safety {
    ...
    match sub {
        ...
        | "show-environment" | "get-default" | "help" | "version" | "daemon-reload"
        | "list-unit-files" => Safety::ReadOnly,
...
mod tests {
    ...
    fn systemctl_read_commands() {
        ...
        assert_eq!(classify_systemctl(&["list-dependencies", "nginx"]), Safety::ReadOnly);
        assert_eq!(classify_systemctl(&["is-active", "nginx"]), Safety::ReadOnly);
        assert_eq!(classify_systemctl(&["is-enabled", "nginx"]), Safety::ReadOnly);
    ...
    fn systemctl_write_commands() {
        assert_eq!(classify_systemctl(&["start", "nginx"]), Safety::WriteOperation);
        assert_eq!(classify_systemctl(&["stop", "nginx"]), Safety::WriteOperation);
        assert_eq!(classify_systemctl(&["restart", "nginx"]), Safety::WriteOperation);
        assert_eq!(classify_systemctl(&["reload", "nginx"]), Safety::WriteOperation);
        assert_eq!(classify_systemctl(&["enable", "nginx"]), Safety::WriteOperation);
        assert_eq!(classify_systemctl(&["disable", "nginx"]), Safety::WriteOperation);
        assert_eq!(classify_systemctl(&["mask", "nginx"]), Safety::WriteOperation);
        assert_eq!(classify_systemctl(&["unmask", "nginx"]), Safety::WriteOperation);
        assert_eq!(classify_systemctl(&["set-default", "multi-user.target"]), Safety::WriteOperation);
        assert_eq!(classify_systemctl(&["set-property", "nginx", "CPUShares=500"]), Safety::WriteOperation);
        assert_eq!(classify_systemctl(&["edit", "nginx"]), Safety::WriteOperation);
        assert_eq!(classify_systemctl(&["reenable", "nginx"]), Safety::WriteOperation);
        assert_eq!(classify_systemctl(&["kill", "nginx"]), Safety::WriteOperation);
    }
}
