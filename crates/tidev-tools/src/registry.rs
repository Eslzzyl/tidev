...
use tidev_config::WebSearchConfig;
...
pub struct ToolRegistry {
    ...
    active_model: Option<tidev_config::ActiveModel>,
...
impl ToolRegistry {
    ...
    pub fn set_active_model(&mut self, model: tidev_config::ActiveModel) {
    ...
    pub fn definitions_for_model(&self, model: &tidev_config::ActiveModel) -> Vec<ToolDefinition> {
        ...
        definitions.extend(self.mcp.all_definitions().into_iter().map(Into::into));
        definitions
    }
    ...
    pub fn mcp_summaries(&self) -> Vec<tidev_mcp::McpServerSummary> {
        self.mcp.summaries()
    }
    ...
    pub fn permission_key_for_call(&self, call: &ToolCall) -> String {
        if call.name == "skill" {
            if let Ok(args) = serde_json::from_str::<crate::SkillArgs>(&call.arguments)
