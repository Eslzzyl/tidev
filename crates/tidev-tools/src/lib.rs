...
// ── Conversion from MCP ToolDefinition ──────────────────────────

impl From<tidev_mcp::ToolDefinition> for ToolDefinition {
    fn from(mcp_def: tidev_mcp::ToolDefinition) -> Self {
        Self {
            name: mcp_def.name,
            display_name: mcp_def.display_name,
            description: mcp_def.description,
            parameters: mcp_def.parameters,
            permission: mcp_def.permission,
            origin: ToolOrigin::Mcp {
                server_name: mcp_def.server_name,
                tool_name: mcp_def.remote_tool_name,
            },
        }
    }
}
