...
/// Per-session event emitted by the agent runtime.
///
/// In the Per-Session Event Bus architecture, each session has its own event
/// channel. This enum therefore carries NO `session_id` field — the receiver
/// already knows which session the events belong to.
///
/// Subagent events (SubagentStatus, SubagentToolResult, SubagentCompleted)
/// are eliminated: the frontend subscribes directly to the child session's
/// event channel.
#[derive(Clone, Debug)]
pub enum BackendEvent {
    ...
    /// Streaming reasoning/thinking delta from the LLM.
    ReasoningDelta {
        request_id: u64,
        content: String,
    },
    /// Tool call state updated (e.g. partial arguments from streaming).
    ToolCallUpdated {
        request_id: u64,
        tool_call: ToolCall,
    },
    /// LLM turn finished — contains either final text or tool calls to execute.
    Finished {
        request_id: u64,
        turn: AssistantTurn,
    },
    /// LLM request failed (will be retried if retry logic is configured).
    Failed {
        request_id: u64,
        error: String,
    },
    ...
    /// Instruction files loaded for the current session.
    InstructionsLoaded {
        sources: Vec<String>,
    },
    ...
    /// Token/model usage statistics for a completed request.
    UsageStats {
        request_id: u64,
        input_tokens: u32,
        output_tokens: u32,
    ...
    /// Context compaction completed (automatic or manual).
    ContextCompacted {
        compacted: bool,
        manual: bool,
...
impl BackendEvent {
    /// Extract the request ID from variants that carry one.
...
mod tests {
    ...
    #[test]
    fn backend_event_exposes_request_id() {
