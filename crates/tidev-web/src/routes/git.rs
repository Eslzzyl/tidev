...
) -> Result<Json<StatusResponse>, crate::error::AppError> {
...
) -> Result<Json<GitBranchResponse>, crate::error::AppError> {
    let cwd = workspace(&state).clone();
...
) -> Result<Json<GitLogResponse>, crate::error::AppError> {
    let cwd = workspace(&state).clone();
...
) -> Result<Json<GitShowResponse>, crate::error::AppError> {
    let cwd = workspace(&state).clone();
...
) -> Result<Json<Vec<GitFileDiffResponse>>, crate::error::AppError> {
    let cwd = workspace(&state).clone();
...
async fn git_diff_file(
...
) -> Result<Json<GitFileDiffResponse>, crate::error::AppError> {
    let cwd = workspace(&state).clone();
    ...
    let output = run_git(&args, &cwd).map_err(crate::error::AppError::Internal)?;
...
async fn git_branch_create(
    ...
    run_git(&["branch", &req.name], &cwd).map_err(crate::error::AppError::Internal)?;

    if req.checkout {
        run_git(&["checkout", &req.name], &cwd).map_err(crate::error::AppError::Internal)?;
    }
...
async fn git_branch_delete(
    ...
    run_git(&["branch", "-D", &name], &cwd).map_err(crate::error::AppError::Internal)?;
...
async fn git_push(
    ...
    run_git(&args, &cwd).map_err(crate::error::AppError::Internal)?;
...
async fn git_pull(
    ...
    run_git(&["pull", remote, branch], &cwd).map_err(crate::error::AppError::Internal)?;
...
async fn git_stash_pop(
    ...
    run_git(&["stash", "pop"], &cwd).map_err(crate::error::AppError::Internal)?;
