...
impl App {
    ...
    pub(crate) fn toggle_stats_panel(&mut self) {
        if let Some(panel) = &mut self.stats_panel {
            ...
            let mut panel = crate::ui::stats_panel::StatsPanelState::new();
            panel.active = true;
    ...
    pub(crate) fn open_balance_panel(&mut self, runtime: &Runtime) -> Result<()> {
        ...
        let mut panel = crate::ui::balance_panel::BalancePanelState::new();
        ...
        *self.balance_panel.lock().unwrap() = Some(panel);
        ...
        let provider_id = match selected_provider {
            crate::ui::balance_panel::ProviderTab::DeepSeek => "deepseek",
            crate::ui::balance_panel::ProviderTab::SiliconFlow => "siliconflow-cn",
        };
            ...
            runtime.spawn(async move {
                match selected_provider {
                    crate::ui::balance_panel::ProviderTab::DeepSeek => {
                        match tidev_session::balance::query_deepseek_balance(&http, &api_key_clone)
                            .await
                        {
                            Ok(balance) => {
