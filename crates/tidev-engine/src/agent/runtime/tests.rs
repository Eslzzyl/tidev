...
use super::AgentRuntime;
    ...
    fn build_request_messages_injects_orphan_tool_failures() {
        ...
        assert!(orphan_tool.is_some(), "orphan tool should become a Tool result");
    ...
    fn build_request_messages_removes_orphan_tool_calls_without_results() {
        ...
        conv.messages = msgs;
        let result = ContextManager::new().build_request_messages(&conv, SessionMode::Build);
        let tool_results: Vec<_> = result.iter().filter(|m| m.role == MessageRole::Tool).collect();
    ...
    fn build_request_messages_removes_orphan_tool_calls_across_messages() {
        ...
        let tools: Vec<_> = result.iter().filter(|m| m.role == MessageRole::Tool).collect();
    ...
    fn build_request_messages_handle_tool_result_matching() {
        ...
        conv.messages = msgs;
        let result = ContextManager::new().build_request_messages(&conv, SessionMode::Build);
        let tool_messages: Vec<_> = result.iter().filter(|m| m.role == MessageRole::Tool).collect();
