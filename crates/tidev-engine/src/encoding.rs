...
// ---------------------------------------------------------------------------
// Windows code page detection & conversion
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    #[test]
    #[cfg(windows)]
    fn test_non_utf8_falls_back_to_codepage() {
        // GBK bytes for "你好世界"
        let gbk = [0xc4, 0xe3, 0xba, 0xc3, 0xca, 0xc0, 0xbd, 0xe7];
        let result = decode_command_output(&gbk);

        // On a Chinese Windows system (CP-936), this should decode correctly.
        // On other Windows systems, it will use the system's code page,
        // which may produce different output.
        let cp = unsafe { GetACP() };
        if cp == 936 {
            assert_eq!(result, "你好世界");
        } else {
            // Not CP-936 — result should at least not contain replacement chars
            assert!(
                !result.contains('\u{FFFD}'),
                "Output should not contain replacement characters on any code page: {result}"
            );
        }
    }
    ...
    fn test_codepage_to_encoding_mapping() {
        // Verify common code pages have valid mappings
        ...
        // Unknown code page should return None
