...
pub(crate) fn extract_boundary_violation_path(
    ...
    let canonical_name = tidev_types::tools::canonical_tool_name(&tool_call.name)?;
    ...
    let path_buf: PathBuf = match canonical_name {
        ...
        "apply_patch" => {
            ...
            PathBuf::from(tidev_utils::path::extract_file_path_from_patch(patch)?)
        }
        "bash" => return None,
        _ => return None,
    };
    ...
    fn resolve_workspace_boundary_dialog(
        ...
        if allowed {
            ...
            if dialog.pending.tool_call.name == "question" {
                let args = match serde_json::from_str::<tidev_types::tools::QuestionArgs>(
