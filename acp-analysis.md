...
- 官方 Rust SDK：[`agent-client-protocol`](https://crates.io/crates/agent-client-protocol)（当前 v2.0.0，schema v1.5.0）
...
├── tidev-acp       — ACP 适配器（Agent Builder + event translator + permission bridge）
├── tidev-llm       — LLM 客户端
...
tidev-acp → tidev-core → tidev-llm
                       → tidev-tools
                       → tidev-storage
                       → tidev-config
                       → tidev-snapshot
...
| `src/main.rs:176-237` | CLI 入口（含 `tidev acp` 子命令） |
| `crates/tidev-acp/src/handler.rs` | ACP agent 入口（Builder + handlers） |
| `crates/tidev-acp/src/event_translator.rs` | BackendEvent → SessionNotification 翻译器 |
